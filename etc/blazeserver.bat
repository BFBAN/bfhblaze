...
for %%C in (%LOCAL_CONFIG_LIST%) do (
	...
	if "%LOCAL_NEW_WINDOW%"=="N" (
		cmd /c ..\out\win32\%LOCAL_MODE%\bin\blazeserver %LOCAL_DESTRUCTIVE% %%C
		goto QUIT
	) else if "%LOCAL_NO_TERMINATE_WHEN_END%"=="Y" (
		start "%%C" cmd /k ..\out\win32\%LOCAL_MODE%\bin\blazeserver %LOCAL_DESTRUCTIVE% %%C
	) else (
		start "%%C" ..\out\win32\%LOCAL_MODE%\bin\blazeserver %LOCAL_DESTRUCTIVE% %%C
	)
)
